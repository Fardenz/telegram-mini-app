...
export default class WebAppServer {
  ...
  constructor(
    private readonly walletEndpoints: WalletEndpoints,
    private readonly gameEndpoints: GameEndpoints
  ) {
    ...
    // OpenAPI UI
    app.use("/api-documentation-ui", SwaggerUi.serve, SwaggerUi.setup(require(apiDocPath), {}))

    this.server = app.listen(port, () => {
      console.log(`⚡️[server]: Server is running at http://localhost:${port}`)
      console.log(
        `⚡️[server]: Documentation available at http://localhost:${port}/api-documentation-ui`
      )
    })
