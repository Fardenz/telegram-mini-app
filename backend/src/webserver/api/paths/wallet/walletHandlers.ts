...
import {
  GetWalletResponsee as GetWalletResponse,
  PaymentLinkResponseBody,
  PostWithdrawMoneyRequestBody,
  PostWithdrawMoneyResponse,
...
export default class WalletEndpoints {
  ...
  public async getWallet(
    ...
    res: Response<GetWalletResponse | ErrorResponse>
  ) {
    try {
  ...
  public async postWithdrawMoney(
    req: CustomExpressRequest<unknown, unknown, PostWithdrawMoneyRequestBody>,
    res: Response<PostWithdrawMoneyResponse | ErrorResponse>
  ) {
    try {
      ...
      if (
        user === null ||
        new Decimal(user?.walletAmountInCents ?? 0).lessThan(req.body.amountInCents)
      ) {
        throw new CustomError("Not enough balance")
      }

      user.walletAmountInCents = new Decimal(user?.walletAmountInCents ?? 0)
        .sub(req.body.amountInCents)
        .toNumber()
  ...
  public async getPaymentLink(
    req: CustomExpressRequest<{ amount: number }, unknown, unknown>,
    res: Response<PaymentLinkResponseBody | ErrorResponse>
  ) {
    if (!req.query.amount) {
